...
import org.locationtech.geomesa.curve.{BinnedTime, Z3SFC, Z3SFCEx}
...
trait Z3IndexKeySpace extends IndexKeySpace[Z3IndexValues, Z3IndexKey] {
  ...
  override def toIndexKey(sft: SimpleFeatureType, lenient: Boolean): SimpleFeature => Seq[Z3IndexKey] = {
    ...
    val z3 = if (xBounds == (180, 180) && yBounds == (-90, 90)) {
      Z3SFCEx(sft.getZ3Interval, 21, xBounds, yBounds)
    } else {
      Z3SFC(sft.getZ3Interval)
    }
